...
// Задание 2
/*Создайте функцию, которая в качестве параметров принимает массив строк и строку. Возвращать данная / функция должна новый массив, содержащий только те элементы переданного массива, которые начинаются с 
переданной строки. Регистр символов не должен влиять.
Пример вызова:
...
let searchStart = (['Кошка', 'Кит', 'Комар', 'Носорог'], 'ко');

const newLine = searchStart.split(' ', 2).join();

searchStart.forEach((newSearch) => {
    if (newSearch.toLowerCase().startsWith(newLine.toLowerCase())) {
    }
});

console.log(newSearch); //полная дичь, скобки в первой строке явно лишние
